...
def model_train(args, net, train_loader, valid_loader):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        ################### Train #################
        outputs_train = run_epoch(net, train_loader, optimizer, 'Training', True, criterion, epoch)
        print('\n')
        ...
        ################### Valid #################
        outputs_valid = run_epoch(net, valid_loader, None, 'Validating', False, criterion, epoch)
        print('\n')
...
def run_epoch(net, data, optimizer, desc, train=False, criterion=None, epoch=0):
    if train:
        net.train()
        optimizer.zero_grad()
    else:
    ...
    for batch in tqdm(data, mininterval=0.5, desc=desc, leave=False, ncols=50):
        ...
        if train:
            pred, pred_masks, masks_new, coor_loss = net(images.cuda(), masks.cuda(), val=False)
        else:
            with torch.no_grad():
                pred, pred_masks, masks_new, coor_loss = net(images.cuda(), masks.cuda(), val=True)

        loss = criterion(pred, masks_new)

        if epoch < 20:
            loss_total.append(coor_loss.item())
        else:
        ...
        for i in range(pred.shape[0]):
            ...
            all_targets.append(masks_new[i, :, :].data.cpu())
...
if __name__ == '__main__':
    ...
    # net = Customize_Model()
    net = Customize_Model_TrainingBox()
    ...
    # checkpoint = torch.load('results/postate158.OurModel.CrossAttentionFeatureFuseJointTrainPromptAndCoarseSeg' + '/best_model.pt')
